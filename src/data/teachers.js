import Babe1 from '../../public/images/babe-1.png';
import Babe2 from '../../public/images/babe-2.png';
import Babe3 from '../../public/images/babe-3.png';
import Babe4 from '../../public/images/babe-4.jpg';
import Babe5 from '../../public/images/babe-5.jpg';
import Babe6 from '../../public/images/babe-6.png';
import Babe7 from '../../public/images/babe-7.jpg';
import Babe8 from '../../public/images/babe-8.jpg';
import BabeBaner from '../../public/images/teacher-banner.png';
import ChemistryTeacher from '../../public/images/teacher-chemistry.jpg';
import BiologyTeacher from '../../public/images/teacher-biology.jpg';

const chemistryTeacher = {
	name: 'Lê Phạm Thành',
	role: 'Giáo viên môn Hóa học tại OnThi247',
	image: ChemistryTeacher,
	school: '',
	subject: '',
	experience: -1,
	bio: [
		'[Lê Phạm Thành]',
		'THPT Chu Văn An, Hà Nội',
		'Môn: Hóa học',
		'[2,600] lượt đăng ký học',
		'Học vị: Cử nhân',
	],
	summary: '',
	description: `Vững về chuyên môn, dày dặn về kinh nghiệm, đam mê với nghề, thầy Lê Phạm Thành đã xây dựng thành công cho riêng mình phong cách giảng dạy độc đáo, mang tính khoa học cao, hệ thống bài tập chặt chẽ, cách truyền đạt thân thiện, dễ hiểu. Với hơn 2 năm kinh nghiệm luyện thi môn Hóa học, thầy đã đạt nhiều thành tích trên con đường dạy học.
	- Giáo viên có số lượng học sinh học online đông nhất Việt Nam hiện nay với rất nhiều thành tích đáng khích lệ như: Có HS đạt danh hiệu thủ khoa toàn quốc năm 2015 (Ngô Vương Minh, chuyên tin, ĐH SPHN); được học sinh yêu mến với biệt danh “thầy giáo thân thiện nhất Vịnh Bắc Bộ”.
	- Tốt nghiệp khoa Hóa học – Trường Đại học Sư phạm Hà Nội.
	- Có 5 năm liên tục dẫn đội tuyển học sinh giỏi đi thi đều đạt giải cấp thành phố với đầy đủ các giải: Nhất, Nhì, Ba, Khuyến khích.
	- Là thành viên chính thức Hội đồng xây dựng ngân hàng đề thi Quốc gia môn Hóa học năm 2009.
	- Liên tục nhiều năm có số học sinh đạt điểm tuyệt đối môn Hóa học trong kì thi THPT QG chiếm tỷ lệ lớn.`,
};

const biologyTeacher = {
	name: 'Thịnh Văn Nam',
	role: 'Giáo viên phụ trách môn Sinh học tại OnThi247',
	image: BiologyTeacher,
	school: '',
	subject: '',
	experience: -1,
	bio: [
		'[6] năm kinh nghiệm',
		'[5/5] sao đánh giá',
		'[1,200] lượt đánh giá',
		'[2,600] lượt đăng ký học',
		'[3] khóa học',
	],
	summary: '',
	description: `Vững về chuyên môn, dày dặn về kinh nghiệm, đam mê với nghề, thầy Thịnh Nam đã xây dựng thành công cho riêng mình phong cách giảng dạy độc đáo, mang tính khoa học cao, hệ thống bài tập chặt chẽ, cách truyền đạt thân thiện, dễ hiểu.Với bề dày hơn 16 năm kinh nghiệm luyện thi môn Sinh học, thầy đã đạt nhiều thành tích trên con đường dạy học.
    - Giáo viên có số lượng học sinh học online đông nhất Việt Nam hiện nay với rất nhiều thành tích đáng khích lệ như: Có HS đạt danh hiệu thủ khoa toàn quốc năm 2015 (Ngô Vương Minh, chuyên tin, ĐH SPHN); được học sinh yêu mến với biệt danh “thầy giáo thân thiện nhất Vịnh Bắc Bộ”.
    - Tốt nghiệp khoa Sinh học – Trường Đại học Sư phạm Hà Nội.
    - Giáo viên bước vào môi trường dạy luyện thi môn sinh ngay từ những ngày đầu là sinh viên sư phạm.
    - Có 5 năm liên tục dẫn đội tuyển học sinh giỏi đi thi đều đạt giải cấp thành phố với đầy đủ các giải: Nhất, Nhì, Ba, Khuyến khích.
    - Là thành viên chính thức Hội đồng xây dựng ngân hàng đề thi Quốc gia môn Sinh học năm 2009.
    - Liên tục nhiều năm có số học sinh đạt điểm tuyệt đối môn Sinh trong kì thi THPT QG chiếm tỷ lệ lớn.`,
};

const englishBabeDescription = `Vững về chuyên môn, dày dặn về kinh nghiệm, đam mê với nghề, cô Đặng Tú Anh đã xây dựng thành công cho riêng mình phong cách giảng dạy độc đáo, mang tính khoa học cao, hệ thống bài tập chặt chẽ, cách truyền đạt thân thiện, dễ hiểu. Với hơn 2 năm kinh nghiệm luyện thi môn Tiếng Anh, thầy đã đạt nhiều thành tích trên con đường dạy học.
📌 Giáo viên có số lượng học sinh học online đông nhất Việt Nam hiện nay với rất nhiều thành tích đáng khích lệ như: Có HS đạt danh hiệu thủ khoa toàn quốc năm 2015 (Ngô Vương Minh, chuyên tin, ĐH SPHN); được học sinh yêu mến với biệt danh “thầy giáo thân thiện nhất Vịnh Bắc Bộ”.
📌 Tốt nghiệp khoa Sinh học – Trường Đại học Sư phạm Hà Nội.
📌 Giáo viên bước vào môi trường dạy luyện thi môn sinh ngay từ những ngày đầu là sinh viên sư phạm.
📌 Có 5 năm liên tục dẫn đội tuyển học sinh giỏi đi thi đều đạt giải cấp thành phố với đầy đủ các giải: Nhất, Nhì, Ba, Khuyến khích.
📌 Là thành viên chính thức Hội đồng xây dựng ngân hàng đề thi Quốc gia môn Sinh học năm 2009.
📌 Liên tục nhiều năm có số học sinh đạt điểm tuyệt đối môn Sinh trong kì thi THPT QG chiếm tỷ lệ lớn.`;

const englishBabes = [
	{
		name: 'Văn Trịnh Quỳnh Anh',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe1,
		school: 'THPT Gia Định, TP. HCM',
		subject: 'Tiếng Anh',
		experience: 5, // in year
		bio: [
			'[Văn Trịnh Quỳnh Anh]',
			'THPT Gia Định, TP. HCM',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		summary:
			'Nghiêm túc trong công việc, phá cách trong giảng dạy, luôn cập nhật cái mới và ứng dụng vào bài giảng giúp cho mỗi bài giảng luôn thú vị, hấp dẫn và say mê.',
		description: englishBabeDescription,
	},
	{
		name: 'Đặng Tú Anh',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe2,
		school: 'THPT Chu Văn An, Hà Nội',
		subject: 'Tiếng Anh',
		experience: 2, // in year
		summary:
			'Theo cô, học sinh cần được dạy bằng trực quan, mỗi bài giảng giáo viên cần thiết kế sinh động với nhiều hoạt động trải nghiệm giúp các con tăng hứng thú với bài học.',
		bio: [
			'[Đặng Tú Anh]',
			'THPT Chu Văn An, Hà Nội',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		description: englishBabeDescription,
	},
	{
		name: 'Trương Thị Ngọc Mai',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe3,
		school: 'THPT Amsterdam, Hà Nội',
		subject: 'Tiếng Anh',
		experience: 2, // in year
		summary:
			'Hiểu tâm lý học sinh, cách truyền đạt đơn giản tự nhiên, cô luôn sử dụng các câu hỏi gợi mở để kích thích học sinh suy nghĩ tìm tòi giải quyết vấn đề.',
		bio: [
			'[Trương Thị Ngọc Mai]',
			'THPT Amsterdam, Hà Nội',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		description: englishBabeDescription,
	},
	{
		name: 'Đỗ Thị Anh Thư',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe4,
		school: 'THPT Chuyên ĐHSP, Hà Nội',
		subject: 'Tiếng Anh',
		experience: 2, // in year
		summary:
			'Cô truyền cảm hứng và khuyến khích học tập thông qua các hoạt động tương tác, các câu hỏi khám phá giúp các em hình thành khả năng sáng tạo và tư duy độc lập.',
		bio: [
			'[Đỗ Thị Anh Thư]',
			'THPT Chuyên ĐHSP, Hà Nội',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		description: englishBabeDescription,
	},
	{
		name: 'Trương Hoàng Anh',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe5,
		school: 'THPT Chu Văn An, Hà Nội',
		subject: 'Tiếng Anh',
		experience: 5, // in year
		summary:
			'Phong cách giảng dạy chậm, tỉ mỉ, ngôn ngữ đơn giản, dễ hiểu, hình ảnh khách quan và các ví dụ liên hệ thức tiễn giúp học sinh tiếp thu kiến thức chậm mà chắc.',
		bio: [
			'[Trương Hoàng Anh]',
			'THPT Chu Văn An, Hà Nội',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		description: englishBabeDescription,
	},
	{
		name: 'Dương Thị Trang',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe6,
		school: 'THPT H.A.S, Hà Nội',
		subject: 'Tiếng Anh',
		experience: 2, // in year
		summary:
			'Theo cô, học sinh cần được dạy bằng trực quan, mỗi bài giảng giáo viên cần thiết kế sinh động với nhiều hoạt động trải nghiệm giúp các con tăng hứng thú với bài học.',
		bio: [
			'[Dương Thị Trang]',
			'THPT H.A.S, Hà Nội',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		description: englishBabeDescription,
	},
	{
		name: 'Hàn Thị Hải Huyền',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe7,
		school: 'THPT Đoàn Thị Điểm, Hà Nội',
		subject: 'Tiếng Anh',
		experience: 2, // in year
		summary:
			'Kinh nghiệm giảng dạy và luyện thi từ căn bản đến nâng cao cho nhiều trình độ khác nhau. Phong cách giảng dạy vui nhộn, trẻ trung tạo cảm hứng cho các em HS.',
		bio: [
			'[Hàn Thị Hải Huyền]',
			'THPT Đoàn Thị Điểm, Hà Nội',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		description: englishBabeDescription,
	},
	{
		name: 'Đào Thị Hồng Nhung',
		role: 'Giáo viên môn Tiếng Anh tại OnThi247',
		image: Babe8,
		school: 'THPT Chuyên ĐHSP, Hà Nội',
		subject: 'Tiếng Anh',
		experience: 2, // in year
		summary:
			'Với phương châm "lấy học sinh làm trung tâm" cô mong muốn truyền động lực học tập để từ đó khơi dậy tính chủ động và nội lực tiềm tàng của các em học sinh.',
		bio: [
			'[Đào Thị Hồng Nhung]',
			'THPT Chuyên ĐHSP, Hà Nội',
			'Môn: Tiếng Anh',
			'Học vị: Cử nhân',
		],
		description: englishBabeDescription,
	},
];

const babeDetail = {
	...englishBabes[1],
	rating: '5/5',
	ratings: 1200,
	students: 2600,
	courses: 2,
	banner: BabeBaner,
}

export { chemistryTeacher, biologyTeacher, englishBabes, babeDetail };
